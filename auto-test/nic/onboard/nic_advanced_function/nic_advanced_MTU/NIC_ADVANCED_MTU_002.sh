...
function test_case()
{
	...
	do	
		#ip link set dev $net mtu 1500
		if [ `cat /sys/class/net/${net}/mtu` -eq 1500 ];then
			PRINT_LOG "INFO" "This $net port mtu value is 1500."
			fn_writeResultFile "${RESULT_FILE}" "$net mtu 1500" "pass"
		else
			PRINT_LOG "FATAL" "This $net port mtu value is not 1500."
			fn_writeResultFile "${RESULT_FILE}" "$net mtu 1500" "fail"
		fi
	done
