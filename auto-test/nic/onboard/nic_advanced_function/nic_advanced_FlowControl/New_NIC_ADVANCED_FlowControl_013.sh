...
#*****************************************************************************************
# *用例名称：New_NIC_ADVANCED_FlowControl_013                                                      
# *用例功能：网卡PAUSE No状态检查                               
# *作者：lwx588815                                                          
# *完成时间：2019-4-10                                                               
# *前置条件：                                                                            
...
# *测试步骤：                                                                               
#      1、测试端执行ethtool -A ethx rx off tx off
#      2、测试端执行ethtool ethx查看PAUSE状态，有结果A
#      3、测试端执行ethtool -A ethx rx on tx on
# *测试结果：                                                                            
#      1.Advertised pause frame use: No                                                    
#*****************************************************************************************
...
{ 
    ...
    do
       ...
       ethtool -A $j rx off tx off 
       sleep 5
       PAUSE=`ethtool $j|grep "Advertised pause frame use:"|awk '{print $5}'`
       if [ "$PAUSE"x = "No"x ];then
           PRINT_LOG "INFO" "$i have No"
           fn_writeResultFile "${RESULT_FILE}" "$i have No" "pass"
       else
           PRINT_LOG "FATAL" "$i not have No"
           fn_writeResultFile "${RESULT_FILE}" "$i not have No" "fail"
       fi
